...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await supabase.from("workspace_user_roles").upsert(
      {
        user_id: userId,
    ...
    if (chatsError) {
      ...
      return NextResponse.json(
        { error: "Failed to assign chat permissions" },
        { status: 500 }
      );
    }
    ...
    const chatRole =
      workspaceToChatRole[invite.role as keyof typeof workspaceToChatRole];
    if (!chatRole) {
      return NextResponse.json(
        { error: "Invalid role mapping" },
        { status: 400 }
